...
class RestInvestServiceIntegrationTests {
	...
	void testGetInvestQuoteString() {
		...
		try {
			security = this.restInvestService.getAndSaveWeeklySecurityPrice(securityId)
...
//		log.info "got security: " + security?.id + " and symbol: " + security?.currentSymbol + " and latest price: " + security.currentPrice
	}
